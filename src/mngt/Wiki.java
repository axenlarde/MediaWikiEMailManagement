...
public class Wiki implements Serializable
{
    ...
    protected String scriptPath = "/wiki/mediawiki";
    ...
    /**
     *  Method use to get the users' data
     */
    public String getUserData() throws IOException
    	{
        String url = query + "meta=userinfo&uiprop=options%7Crealname%7Cemail";
        return fetch(url, "getUserData");
    	}
    
    /******
     *  Method use to get the categories list
     */
    public ArrayList<String> getCategorieList() throws Exception
    	{
        String url = query + "list=allcategories&acprop=hidden";
        String catList = fetch(url, "getCategorieList");
        
        ArrayList<String> arg = new ArrayList<String>();
		arg.add("query");
		arg.add("allcategories");
		return utils.xMLGear.getResultList(catList, arg);
    	}
    
    /******
     *  Method use to get the Unused categories list
     */
    public ArrayList<String> getUnusedCategorieList() throws Exception
    	{
    	String url = query + "list=querypage&qppage=Unusedcategories";
        String unUsedCatList = fetch(url, "getUnusedCategorieList");
        
        unUsedCatList = unUsedCatList.substring(unUsedCatList.indexOf("<results"), unUsedCatList.indexOf("</results>")+10);
        ArrayList<String> catList = new ArrayList<String>();
        
        //On parse le contenu XML
        SAXBuilder sxb = new SAXBuilder();
		Element root = sxb.build(new InputSource(new StringReader(unUsedCatList))).getRootElement();
        
		Iterator i = root.getChildren().iterator();
		while(i.hasNext())
			{
			Element courant = (Element)i.next();
			catList.add(courant.getAttributeValue("value"));
			}
    ...
    /**
     *  Method used to get a lit containing categories
     */
    /*
    public String[] listCategories() throws IOException, CredentialNotFoundException
    	{
        if ((user == null || !user.isAllowedTo("unwatchedpages")))
            throw new CredentialNotFoundException("User does not have the \"unwatchedpages\" permission.");
        
        String url = query + "action=query&list=allcategories"
        String offset = "";
        ArrayList<String> pages = new ArrayList<String>(1333);
    ...
    protected void log(Level level, String text, String method)
    {
        ...
        variables.getLogger().info(sb.toString());
    }
